...
/* 要求的是「明白推薦引線」這件事，不是某句固定措辭。
 * 原本寫死「請直接用引線」，但那句把兩者說成互斥的替代關係，
 * 與對照表（引線同樣支援全台 22 縣市地方法規）不符，已改為互補的說法。 */
ok('建議能裝擴充功能者也試試引線',
   text.includes('也可以安裝「引線」比較看看') && text.includes('主動掃描'));
/* 不得宣稱本工具有引線沒有的地方法規支援——引線功能說明明載
